import { normalizeTrustedOrigin } from "@nearzero/server";
import { db } from "@nearzero/server/db";
import { ssoProvider, user } from "@nearzero/server/db/schema";
import { ssoProviderBodySchema } from "@nearzero/server/db/schema/sso";
import {
	getOrganizationOwnerId,
	requestToHeaders,
} from "@nearzero/server/index";
import { auth } from "@nearzero/server/lib/auth";
import {
	EDITION_FEATURES,
...
import {
	createTRPCRouter,
	paidFeatureProcedure,
	publicProcedure,
} from "@/server/api/trpc";

export const ssoRouter = createTRPCRouter({
	showSignInWithSSO: publicProcedure.query(async () => {
		return isEditionFeatureEnabled(EDITION_FEATURES.sso);
	}),
	listProviders: paidFeatureProcedure(EDITION_FEATURES.sso).query(
		async ({ ctx }) => {
	...
	getTrustedOrigins: paidFeatureProcedure(EDITION_FEATURES.sso).query(
		async ({ ctx }) => {
	...
	one: paidFeatureProcedure(EDITION_FEATURES.sso)
	...
	update: paidFeatureProcedure(EDITION_FEATURES.sso)
	...
	deleteProvider: paidFeatureProcedure(EDITION_FEATURES.sso)
		.input(z.object({ providerId: z.string().min(1) }))
		.mutation(async ({ ctx, input }) => {
	...
	register: paidFeatureProcedure(EDITION_FEATURES.sso)
		.input(ssoProviderBodySchema)
		.mutation(async ({ ctx, input }) => {
	...
	addTrustedOrigin: paidFeatureProcedure(EDITION_FEATURES.sso)
	...
	removeTrustedOrigin: paidFeatureProcedure(EDITION_FEATURES.sso)
		.input(z.object({ origin: z.string().min(1) }))
		.mutation(async ({ ctx, input }) => {
			const ownerId = await getOrganizationOwnerId(
				ctx.session.activeOrganizationId,
	...
	updateTrustedOrigin: paidFeatureProcedure(EDITION_FEATURES.sso)
		.input(
			z.object({
