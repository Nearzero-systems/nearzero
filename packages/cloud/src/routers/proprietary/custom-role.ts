...
import { member, organizationRole, user } from "@nearzero/server/db/schema";
import { statements } from "@nearzero/server/lib/access-control";
...
export const customRoleRouter = createTRPCRouter({
	...
	create: paidFeatureProcedure(EDITION_FEATURES.customRoles)
		.input(
			z.object({
				roleName: z
					.string()
	...
	update: paidFeatureProcedure(EDITION_FEATURES.customRoles)
		.input(
			z.object({
	...
	remove: paidFeatureProcedure(EDITION_FEATURES.customRoles)
		.input(
			z.object({
				roleName: z.string().min(1),
			}),
