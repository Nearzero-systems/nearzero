...
export const stripeRouter = createTRPCRouter({
	...
	getCurrentPlan: protectedProcedure.query(async ({ ctx }) => {
		if (!shouldEnforceCloudBilling()) return null;
	...
	createCheckoutSession: adminProcedure
		...
		.mutation(async ({ ctx, input }) => {
			...
			const session = await stripe.checkout.sessions.create({
				...
				success_url: `${WEBSITE_URL}/dashboard/servers?success=true`,
	...
	updateInvoiceNotifications: adminProcedure
		...
		.mutation(async ({ ctx, input }) => {
			if (!shouldEnforceCloudBilling()) {
				throw new TRPCError({
					code: "BAD_REQUEST",
					message: "Billing is not configured for this installation",
				});
			}
			const owner = await findUserById(ctx.user.ownerId);
			await updateUser(owner.id, {
