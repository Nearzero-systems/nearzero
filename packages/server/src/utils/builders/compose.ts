...
Compose Type: ${composeType} ✅`;
	...
	set -e
	{
		...
		${dockerComposeEnvPrefix} ${exportEnvCommand} docker ${command.split(" ").join(" ")} 2>&1 || { echo "Error: ❌ Docker command failed"; exit 1; }
		${compose.isolatedDeployment ? `docker network connect ${compose.appName} $(docker ps --filter "name=nearzero-traefik" -q) >/dev/null 2>&1` : ""}
