import { CLEANUP_CRON_JOB } from "@nearzero/server/constants";
...
import { getAllServers } from "@nearzero/server/services/server";
...
export const initCronJobs = async () => {
	...
	if (isCommunityMode() && webServerSettings?.enableDockerCleanup) {
		try {
	...
	if (isCommunityMode() && webServerSettings?.logCleanupCron) {
		try {
			console.log(
