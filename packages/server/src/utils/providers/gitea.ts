...
export const refreshGiteaToken = async (giteaProviderId: string) => {
	try {
		...
		assertGitProviderConnectionAllowed(giteaProvider, "Gitea");
		const managedConfig = isNearzeroManagedGitProvider(giteaProvider)
			? getManagedGiteaConfig()
			: null;

		if (
			!(managedConfig?.clientId || giteaProvider?.clientId) ||
		...
		// Use internal URL when Gitea is on same instance as Nearzero
...
export const haveGiteaRequirements = (giteaProvider: Gitea) => {
	if (isHostedEditionMode() && !isNearzeroManagedGitProvider(giteaProvider)) {
		return false;
	}
	if (isNearzeroManagedGitProvider(giteaProvider)) {
		return !!giteaProvider?.accessToken;
	}
