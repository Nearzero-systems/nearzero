...
export const refreshGitlabToken = async (gitlabProviderId: string) => {
	...
	assertGitProviderConnectionAllowed(gitlabProvider, "GitLab");
	const currentTime = Math.floor(Date.now() / 1000);
	const managedConfig = isNearzeroManagedGitProvider(gitlabProvider)
		? getManagedGitlabConfig()
		: null;
	...
	// Use internal URL for token refresh when GitLab is on same instance as Nearzero
...
export const haveGitlabRequirements = (gitlabProvider: Gitlab) => {
	if (isHostedEditionMode() && !isNearzeroManagedGitProvider(gitlabProvider)) {
		return false;
	}
