...
export const restoreWebServerBackup = async (
	...
	try {
		...
		const tempDir = await mkdtemp(join(tmpdir(), "nearzero-restore-"));
		...
		try {
			...
			await execAsync(
				`docker exec ${postgresContainerId} psql -U nearzero postgres -c "SELECT pg_terminate_backend(pg_stat_activity.pid) FROM pg_stat_activity WHERE pg_stat_activity.datname = 'nearzero' AND pid <> pg_backend_pid();"`,
			);

			emit("Dropping existing database...");
			await execAsync(
				`docker exec ${postgresContainerId} psql -U nearzero postgres -c "DROP DATABASE IF EXISTS nearzero;"`,
			);

			emit("Creating fresh database...");
			await execAsync(
				`docker exec ${postgresContainerId} psql -U nearzero postgres -c "CREATE DATABASE nearzero;"`,
			);
			...
			await execAsync(
				`docker exec ${postgresContainerId} pg_restore -v -U nearzero -d nearzero /tmp/database.sql`,
			);
