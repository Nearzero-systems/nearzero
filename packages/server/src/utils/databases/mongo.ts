...
${command ?? "wait $MONGOD_PID"}`;
	...
	await upsertSwarmService({
		...
		appName,
		replicas,
		// MongoDB's first-run init (the official image starts a temporary mongod
		// to create the root user, then restarts) produces a brief "running"
		// window. A churning mongod (e.g. SIGILL on a CPU without AVX, or an OOM
		// kill) can fall inside the default 10s stable window and be falsely
		// reported as a successful deploy. Require a longer continuous-running
		// window (and a larger timeout to accommodate the slower init) so the
		// service must genuinely stay up before the deploy is declared done.
		// This is mongo-only; other database builders keep the default window.
		stabilityOptions: {
			stableMs: 30_000,
			timeoutMs: 120_000,
		},
