import { VALID_BRANCH_REGEX } from "@nearzero/server/utils/git-branch-validation";
...
import { z } from "zod";
import type { BuildSelectionMode } from "../../types/application-build-plan";
...
export const buildExecutionTarget = pgEnum("buildExecutionTarget", [
	"deploy_server",
	"nearzero_host",
]);
...
export const applications = pgTable("application", {
	...
	customInstallCommand: text("customInstallCommand"),
	customBuildCommand: text("customBuildCommand"),
	customStartCommand: text("customStartCommand"),
	...
	buildSelectionMode: text("buildSelectionMode")
		.$type<BuildSelectionMode>()
		.notNull()
		.default("automatic"),
	buildExecutionTarget: buildExecutionTarget("buildExecutionTarget")
		.notNull()
		.default("deploy_server"),
...
const createSchema = createInsertSchema(applications, {
	...
	customBuildCommand: z.string().optional(),
	customStartCommand: z.string().optional(),
	name: z.string().min(1),
	...
	buildSelectionMode: z.enum(["automatic", "explicit"]).default("automatic"),
	buildExecutionTarget: z
...
export const apiSaveBuildCommands = createSchema
	.pick({
		applicationId: true,
		customInstallCommand: true,
		customBuildCommand: true,
		customStartCommand: true,
	})
	.required();
