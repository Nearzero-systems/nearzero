...
import { z } from "zod";
import type { ApplicationBuildPlan } from "../../types/application-build-plan";
...
export const deployments = pgTable("deployment", {
	...
	executionMode: text("executionMode").$type<"cloud" | "community">(),
	buildLocation: text("buildLocation").$type<"local" | "remote">(),
	buildPlan: jsonb("buildPlan").$type<ApplicationBuildPlan>(),
