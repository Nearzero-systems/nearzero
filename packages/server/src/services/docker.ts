...
export const getContainers = async (serverId?: string | null) => {
	try {
		...
		const containers = lines
			...
			.filter(
				(container) =>
					!container.name.includes("nearzero") ||
					container.name.includes("nearzero-monitoring"),
			);
...
export const getNodeApplications = async (serverId?: string) => {
	try {
		...
		const appArray = stdout
			...
			.split("\n")
			.map((line) => JSON.parse(line))
			.filter((service) => !service.Name.startsWith("nearzero-"));
...
export const uploadFileToContainer = async (
	...
	const tempFileName = `nearzero-upload-${Date.now()}-${fileName.replace(/[^a-zA-Z0-9.-]/g, "_")}`;
