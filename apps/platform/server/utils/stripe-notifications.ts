import InvoiceNotificationEmail from "@nearzero/server/emails/emails/invoice-notification";
import PaymentFailedEmail from "@nearzero/server/emails/emails/payment-failed";
import { sendEmail } from "@nearzero/server/verification/send-verification-email";
...
export const sendInvoiceEmail = async (
	...
	try {
		...
		if (invoice.invoice_pdf) {
			...
			if (pdfBuffer) {
				attachments.push({
					filename: `nearzero-invoice-${invoice.number || invoice.id}.pdf`,
					content: pdfBuffer,
				});
			}
		}
		...
		await sendEmail({
			email: admin.email,
			subject: `Nearzero Invoice ${invoice.number || ""} - ${amountFormatted}`,
			text: htmlContent,
			attachments,
		});
...
export const sendPaymentFailedEmail = async (
	...
	try {
		...
		await sendEmail({
			email: admin.email,
			subject: `Action required: Nearzero payment failed - ${amountFormatted}`,
			text: htmlContent,
		});
