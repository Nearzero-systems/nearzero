...
cat /etc/nearzero/ssh/auto_generated-nearzero-local.pub >> $HOME/.ssh/authorized_keys && \\
echo "✓ Nearzero SSH key added successfully. Reopen the terminal in Nearzero to reconnect."
# ----------------------------------------`;

const COMMAND_TO_GRANT_PERMISSION_ACCESS = `
# ----------------------------------------
sudo chown -R $USER:$USER /etc/nearzero/ssh
# ----------------------------------------
`;
...
export const setupTerminalWebSocketServer = (
	...
	wssTerm.on("connection", async (ws, req) => {
		...
		if (isLocalServer) {
			if (!isCommunityMode()) {
				ws.close(4000, "Local host terminal is disabled in Cloud mode.");
				return;
			}
