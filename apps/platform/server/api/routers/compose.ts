...
import { runServiceScaleAction } from "../utils/service-scale";
	...
	stop: protectedProcedure
		...
		.mutation(async ({ input, ctx }) => {
			...
			await assertRuntimePlacement(ctx, "service.stop", {
				serverId: composeForStop.serverId ?? null,
				resourceType: "compose",
				resourceId: composeForStop.composeId,
				resourceName: composeForStop.name,
				serviceType: "compose",
				environmentId: composeForStop.environmentId,
			});
			await runServiceScaleAction("stop", () => stopCompose(input.composeId));
			await audit(ctx, {
				action: "stop",
				resourceType: "compose",
				resourceId: input.composeId,
	...
	start: protectedProcedure
		...
		.mutation(async ({ input, ctx }) => {
			...
			await assertRuntimePlacement(ctx, "service.start", {
				serverId: composeForStart.serverId ?? null,
				resourceType: "compose",
				resourceId: composeForStart.composeId,
				resourceName: composeForStart.name,
				serviceType: "compose",
				environmentId: composeForStart.environmentId,
			});
			await runServiceScaleAction("start", () => startCompose(input.composeId));
			await audit(ctx, {
				action: "start",
				resourceType: "compose",
				resourceId: input.composeId,
