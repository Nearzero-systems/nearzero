...
export const notificationRouter = createTRPCRouter({
	createSlack: withPermission("notification", "create")
		...
		.mutation(async ({ input, ctx }) => {
			try {
				...
				console.error("Failed to create Slack notification");
				throw new TRPCError({
					code: "BAD_REQUEST",
	...
	testTelegramConnection: withPermission("notification", "create")
		...
		.mutation(async ({ input }) => {
			try {
				await sendTelegramNotification(input, "Hi, From Nearzero 👋");
				return true;
			} catch (error) {
				throw new TRPCError({
					code: "BAD_REQUEST",
		...
		.mutation(async ({ input }) => {
			try {
				...
				await sendDiscordNotification(input, {
					...
					description: decorate(">", "Hi, From Nearzero 👋"),
					color: 0xf3f7f4,
				});
	...
	receiveNotification: publicProcedure
		.input(
			z.object({
				ServerType: z.enum(["Nearzero", "Remote"]).default("Nearzero"),
		...
		.mutation(async ({ input }) => {
			try {
				await sendNtfyNotification(
					...
					"Test Notification",
					"",
					"view, visit Nearzero on Github, https://github.com/Nearzero-systems/nearzero, clear=true;",
					"Hi, From Nearzero 👋",
				);
				return true;
			} catch (error) {
		...
		.mutation(async ({ input }) => {
			try {
				await sendMattermostNotification(input, {
					text: "Hi, From Nearzero 👋",
					channel: input.channel,
					username: input.username || "Nearzero Bot",
				});
				return true;
			} catch (error) {
				throw new TRPCError({
		...
		.mutation(async ({ input }) => {
			try {
				await sendTeamsNotification(input, {
					title: "🤚 Test Notification",
					facts: [{ name: "Message", value: "Hi, From Nearzero 👋" }],
				});
				return true;
			} catch (error) {
				throw new TRPCError({
