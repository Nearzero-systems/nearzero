...
export const whitelabelingRouter = createTRPCRouter({
	get: protectedProcedure.query(async () => {
		if (!isEditionFeatureEnabled(EDITION_FEATURES.whitelabeling)) {
			return null;
		}
	...
	update: paidFeatureProcedure(EDITION_FEATURES.whitelabeling)
	...
	reset: paidFeatureProcedure(EDITION_FEATURES.whitelabeling).mutation(
		async ({ ctx }) => {
			if (ctx.user.role !== "owner") {
				throw new TRPCError({
					code: "FORBIDDEN",
