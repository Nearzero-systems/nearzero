...
import type { statements } from "@nearzero/server/lib/access-control";
import { validateRequest } from "@nearzero/server/lib/auth";
import {
	EDITION_FEATURES,
	type EditionFeature,
 ...
 * - testing, so we don't have to mock HTTP req/res
...
export const paidFeatureMiddleware = (feature: EditionFeature) =>
	t.middleware(async ({ ctx, next }) => {
		...
		if (
			!isEditionFeatureEnabled(feature, {
			...
			throw new TRPCError({
				code: "FORBIDDEN",
				message: `${getEditionFeatureLabel(feature)} is not available in Community mode`,
			});
		}
...
export const paidFeatureProcedure = (feature: EditionFeature) =>
	t.procedure.use(paidFeatureMiddleware(feature));

/**
 * Legacy enterprise procedure. Prefer paidFeatureProcedure(feature) for new routes.
 */
export const enterpriseProcedure = paidFeatureProcedure(
	EDITION_FEATURES.managedSupport,
);
