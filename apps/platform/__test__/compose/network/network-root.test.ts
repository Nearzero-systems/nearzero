...
import { addSuffixToNetworksRoot, generateRandomHash } from "@nearzero/server";
...
test("It shouldn't add suffix to nearzero-network", () => {
	...
	expect(networks).toBeDefined();
	for (const networkKey of Object.keys(networks)) {
		expect(networkKey).toContain("nearzero-network");
	}
});
