import type { ComposeSpecification } from "@nearzero/server";
import {
...
test("It shouldn't add suffix to nearzero-network in services", () => {
	...
	expect(service?.networks).toContain("nearzero-network");
});
...
test("It shouldn't add suffix to nearzero-network in services multiples cases", () => {
	...
	expect(redis?.networks).toHaveProperty("nearzero-network");
	expect(dbNetworks["nearzero-network"]).toBeDefined();
	expect(apiNetworks["nearzero-network"]).toBeDefined();
});
