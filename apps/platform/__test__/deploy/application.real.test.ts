...
import type { ApplicationNested } from "@nearzero/server";
import { paths } from "@nearzero/server/constants";
import { execAsync } from "@nearzero/server/utils/process/execAsync";
...
vi.mock("@nearzero/server/services/admin", () => ({
	getConsoleUrl: vi.fn().mockResolvedValue("http://localhost:3000"),
	getNearzeroUrl: vi.fn().mockResolvedValue("http://localhost:3000"),
}));
