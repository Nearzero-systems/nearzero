import type { Registry } from "@nearzero/server";
import { getRegistryTag } from "@nearzero/server";
...
describe("getRegistryTag", () => {
	...
	describe("with username (no imagePrefix)", () => {
		...
		it("should handle complex image name with username", () => {
			const registry = createMockRegistry({ username: "nearzero" });
			const result = getRegistryTag(
				registry,
				"nearzero/app-parse-multi-byte-port-e32uh7",
			);
			// Should not duplicate username
			expect(result).toBe(
				"docker.io/nearzero/app-parse-multi-byte-port-e32uh7",
			);
		});
	...
	describe("special characters in username", () => {
		it("should handle Harbor robot account username with $ (e.g. robot$library+nearzero)", () => {
			const registry = createMockRegistry({
				username: "robot$library+nearzero",
			});
			const result = getRegistryTag(registry, "nginx");
			expect(result).toBe("docker.io/robot$library+nearzero/nginx");
		});
