...
vi.mock("@nearzero/server/constants", async (importOriginal) => {
	const actual =
		await importOriginal<typeof import("@nearzero/server/constants")>();
	return {
		...actual,
		paths: () => ({
			...actual.paths(),
...
const { readValidDirectory } = await import("@nearzero/server");
