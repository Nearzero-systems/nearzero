...
export function buildComposeDeploySpec(input: ComposeDeployWizardInput): {
	...
	const presetDefaults = getFrameworkPresetDefaults(
		String(input.preset || "Other").trim() || "Other",
	);
	...
	const output =
		String(input.outputDir || defaultOutput).trim() || defaultOutput;
	...
	if (type === "static") {
		...
		draft = {
			...
			...(resolvedBuildCommand
				? { build: { command: resolvedBuildCommand } }
				: {}),
			start: {
